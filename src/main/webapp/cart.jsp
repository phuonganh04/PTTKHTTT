...
<head>
<meta charset="UTF-8">
<meta name="description" content="app mua hàng online">
<meta name="keywords" content="app, mua, online">
					...
					<table class="table list-sp" style="background-color: white;">
						...
						<tbody>
							<c:forEach var="item" items="${sessionScope.cart}">
								<tr>
									<td class="san-pham"><a href="book-detail?id=${item.getBook().getId()}"
										style="margin-right: 10px"><img
											src="${item.getBook().getImage() }"
											alt="product"></a><a href="book-detail?id=${item.getBook().getId()}">
											<h6>${item.getBook().getTitle()}</h6>
									</a></td>
									<td><fmt:formatNumber type="number" maxFractionDigits="3"
											value="${item.getPrice()}" /> Đ</td>
									
									<td><fmt:formatNumber type="number" maxFractionDigits="3"
										value="${item.getAmount()}"/></td>
									 <td><fmt:formatNumber type="number" maxFractionDigits="3"
											value="${item.getPrice()*item.getAmount() }" /> Đ</td>
									<td></td> 
